...
class BinaryTree:
    ...
    def sum_of_nodes(self):
        ...
        sum_nodes = 0
        sum_list = []
        return self._sum_of_nodes(sum_nodes, sum_list)


    def _sum_of_nodes(self, sum_nodes, sum_list):
        if self.left_node.value is None and self.right_node.value is None:
            sum_list.append(sum_nodes + self.value)
            # print(sum_nodes + self.value)
            # sum_nodes = 0
        else:
            # print('+', self.value)
            sum_nodes +=  self.value
        if self.left_node.value is not None:
            self.left_node._sum_of_nodes(sum_nodes, sum_list)
        if self.right_node.value is not None:
            # print(self.right_node.value)
            self.right_node._sum_of_nodes(sum_nodes, sum_list)
        return max(sum_list)
        
    def expand_tree(self, max_branch):
        sum_nodes = 0
        sum_list = []
        return self._expand_tree(max_branch, sum_nodes, sum_list)


    def _expand_tree(self, max_branch, sum_nodes, sum_list):
        sum_nodes +=  self.value
        if self.left_node.value is None and self.right_node.value is None:
            # print(sum_nodes)
            if sum_nodes < max_branch:
                # print(max_branch - sum_nodes)
                self.left_node = BinaryTree(int(max_branch - sum_nodes))
                self.right_node = BinaryTree(int(max_branch - sum_nodes))
            # print(sum_nodes + self.value)
        elif self.left_node.value is not None and self.right_node.value is None:
            self.left_node._expand_tree(max_branch, sum_nodes, sum_list)
            if sum_nodes < max_branch:
                self.right_node = BinaryTree(int(max_branch - sum_nodes))
            # print(sum_nodes)
        elif self.left_node.value is None and self.right_node.value is not None:
            self.right_node._expand_tree(max_branch, sum_nodes, sum_list)
            if sum_nodes < max_branch:
                self.left_node = BinaryTree(int(max_branch - sum_nodes))
            # print(sum_nodes)
        elif self.left_node.value is not None and self.right_node.value is not None:
            self.left_node._expand_tree(max_branch, sum_nodes, sum_list)
            self.right_node._expand_tree(max_branch, sum_nodes, sum_list)
        return self
...
if __name__ == '__main__':
    ...
    # t = BinaryTree(3)
    # print(t.sum_of_nodes())
    t.expand_tree(t.sum_of_nodes())
    t.print_binary_tree()
    # print(values)
    # print(um_values)
    # t1 = BinaryTree(2)
    # t1.left_node = BinaryTree(3)
    # t1.right_node = BinaryTree(4)
    # print(t1.sum_of_nodes())
    # print(BinaryTree(4).sum_of_nodes())
